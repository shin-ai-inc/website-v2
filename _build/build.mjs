...
/* styles/app.css は生成物。編集は styles/ 配下の正本で行う。 */
let css = "";
...
/* ソースを開いたときに設計がそのまま読めないよう、公開する結合物は圧縮する。
...
const standalonePages = [
  ...
  },
  {
    ...
    desc: "社員の知恵と業務の流れをもとに、転記や確認、属人化といった手間の出どころを整理し、"
        + "自社に合うAIシステムの開発から定着まで支援する。"
        + "AIを導入するか決まっていない段階からの相談を前提にした案内。",
  },
];
...
const pages = [
  { file: "index.html", part: "index.html", nav: "home", hero: true,
    ...
    /* 「シンアイ株式会社」で検索すると、社名をtitleに含む gunma-ai.html が
       ...
       titleに持つページが全ページ中ここだけだったため。地域ページ以外の
       全ページは "○○｜ShinAI" とブランド表記のみで、正式社名は
       構造化データ(Organization.name)にしか無く、画面に出るtitleには
       一度も出ていなかった。トップページのtitleに正式社名を置き、
       社名検索での本来の入口(トップ)を取り戻す。 */
    /* 社名で直接検索されたときの見え方を優先し、題名は理念の一文に戻す(柴田指示 2026-09-13)。
       「群馬 AI企業」への対策は、題名ではなく本文と構造化データで行う。 */
    title: "シンアイ株式会社｜企業の暗黙知を、使えるAI資産へ",
  ...
  /* 地域ページ。トップの title はブランドの表現として据え置くと決めたため、
     ...
     「群馬県の」→「群馬発の」(柴田指示・2026-08-27)。h1(「群馬発、
     現場で使われるAIを。」)・本文とも既に「群馬発」で統一されており、
     titleだけ「県の」で取り残されていた。「群馬」の語自体は残るため
     地域クエリの手掛かりは失わない。県域限定の主張ではなく出自の
     表明に変わる分、県外の読み手への閉め出し感も和らぐ
     （上のコメントが警戒していた「県内限定に読める」問題とも整合する）。 */
...
let built = 0;
/* 知識ベースは改行を段落の区切りとして読むため、圧縮前の HTML から作る。 */
...
for (const loc of LOCALES) {
  ...
  for (const page of pages) {
    ...
    const html = stripComments(shell(page, loc));
    sourceHtml.set(loc.dir + page.file, html);
    write(loc.dir + page.file, collapseHtml(html));
    built += 1;
  }
}
...
const minifyTree = (dir) => {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) { minifyTree(path); continue; }
    const minify = MINIFIERS[entry.name.slice(entry.name.lastIndexOf("."))];
    if (minify) writeFileSync(path, minify(readFileSync(path, "utf8")), "utf8");
  }
};
for (const p of standalonePages) {
  cpSync(join(ROOT, p.dir), join(DIST, p.dir), { recursive: true });
  minifyTree(join(DIST, p.dir));
}
...
const knowledgeFor = (loc) => {
  ...
  for (const page of pages) {
    ...
    if (!localesFor(page).includes(loc)) continue;
    const meta = loc.code === "ja" ? page : page.en;
    const html = sourceHtml.get(loc.dir + page.file);
    const ctx = {
      title: meta.title,
